...
jest.mock('@pulumi/pulumi', () => ({
  ...
  output: jest.fn((value) => ({
    apply: jest.fn((fn: (val: any) => any) => fn(value)),
  })),
  all: jest.fn((values) => ({
    apply: jest.fn((fn: (vals: any) => any) => fn(values)),
  })),
}));
...
describe('NetworkComponent', () => {
  ...
  describe('Bridge Initialization', () => {
    it('should initialize bridges with correct properties', () => {
      ...
      const bridges = component['initializeBridges'](
        basicNetworkConfig.bridges!
      );
  ...
  describe('VLAN Initialization', () => {
    it('should initialize VLANs and associate with bridges', () => {
      ...
      const vlans = component['initializeVLANs'](
        basicNetworkConfig.vlans!,
        bridges
      );
  ...
  describe('Firewall Rules Initialization', () => {
    it('should initialize firewall rules when enabled', () => {
      const component = new NetworkComponent('test-network', basicProps);
      const rules = component['initializeFirewallRules'](
        basicNetworkConfig.firewall
      );
    ...
    it('should return empty array when firewall is disabled', () => {
      const component = new NetworkComponent('test-network', basicProps);
      const disabledFirewall = {
        enabled: false,
        defaultPolicy: 'accept' as const,
        rules: [],
      };
  ...
  describe('IP Range Initialization and Parsing', () => {
    ...
    it('should calculate IP addresses correctly', () => {
      ...
      expect(component['calculateIPAddress']([192, 168, 1, 0], 1)).toBe(
        '192.168.1.1'
      );
      expect(component['calculateIPAddress']([192, 168, 1, 0], 255)).toBe(
        '192.168.1.255'
      );
      expect(component['calculateIPAddress']([10, 0, 0, 0], 256)).toBe(
        '10.0.1.0'
      );
      expect(component['calculateIPAddress']([10, 0, 0, 0], 65536)).toBe(
        '10.1.0.0'
      );
    });
  });
  ...
  describe('IP Allocation Methods', () => {
    it('should allocate IP from VLAN', () => {
      ...
      (component.vlans as any).apply = jest.fn((fn) =>
        fn([{ id: 10, name: 'test', ipRange: '192.168.1.0/24' }])
      );
  ...
  describe('Configuration Validation', () => {
    ...
    it('should validate network configuration', () => {
      ...
      expect(pulumi.all).toHaveBeenCalledWith([
        component.bridges,
        component.vlans,
      ]);
    });
